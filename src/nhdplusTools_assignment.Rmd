...
NF_UM_comids <- get_UM(network=nofo_flowlines, comid= (flowlines_for_pathlengths %>% st_set_geometry(., NULL) %>% filter(toID==0) %>% select(ID))[[1]]
                         ) %>%
  as_tibble() %>%
  rename(comid=value)
...
gages <- gages %>% left_join(NF_flowlines, by= "comid") %>% 

...
```{r}
# upstream_of_end_comids <- get_UM(
#   network=nofo_flowlines
#   , comid= "8441253" # N F SHENANDOAH RIVER NEAR STRASBURG, VA 
#   ) %>%
#   as_tibble() %>%
#   rename(comid=value)
# 
# downstream_of_st_comids <- get_DM(
#   network=nofo_flowlines
#   , comid= "8441037" # N F SHENANDOAH RIVER AT COOTES STORE, VA
#   ) %>%
#   as_tibble() %>%
#   rename(comid=value)
# 
# 
# flowlines_for_gages <- filter(nofo_flowlines, comid %in% c(upstream_of_end_comids$comid)
#                                  & comid %in% c(downstream_of_st_comids$comid)) 
# 
# 
# 
# flowlines_for_gages_comid <- get_tocomid(flowlines_for_gages, add=TRUE) %>%
#   rename(ID=comid, toID=tocomid, length=lengthkm) %>% 
#   select(ID, toID, length) %>% 
#   st_set_geometry(., NULL)
# 
# distances_gages <- get_pathlength(flowlines_for_gages_comid) %>% 
#   rename(comid=ID,
#          distance_km=pathlength)
...
#USGS stream gages
gages <- get_nwis(AOI=clp_watershed)

mapview(gages) + mapview(clp_flowlines) + mapview(clp_watershed) 
